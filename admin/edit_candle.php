...
if(!isset($_GET['id'])) {
    echo "<script>window.location.href='candles.php';</script>";
    exit;
}

$id = intval($_GET['id']);
$item_query = mysqli_query($conn, "SELECT * FROM candles WHERE id = $id");
$item = mysqli_fetch_assoc($item_query);

if(!$item) {
    echo "<script>alert('Candle not found!'); window.location.href='candles.php';</script>";
    exit;
}

if(isset($_POST['update_candle'])) {
    ...
    if(mysqli_query($conn, $q)) {
        echo "<script>alert('Candle updated successfully!'); window.location.href='candles.php';</script>";
    } else {
        echo "<script>alert('Failed to update candle.');</script>";
    }
}
?>
...
<div class="modern-form-container">
    <h2 class="modern-form-title">✏️ Edit Candle ✏️</h2>
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="modern-form-group">
            ...
            <img src="../assets/images/candles/<?php echo $item['image']; ?>" width="80" style="margin-top:10px; border-radius:5px;">
        </div>
        ...
        <button type="submit" name="update_candle" class="modern-btn-submit">Update Candle</button>
    </form>
</div>
