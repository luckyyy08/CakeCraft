...
if (isset($_POST['add_decoration'])) {
    ...
    $q = "INSERT INTO decorations (name, price, original_price, image, description) VALUES ('$name', '$price', '$original_price', '$image', '$description')";

    if (mysqli_query($conn, $q)) {
        echo "<script>alert('Decoration added successfully!'); window.location.href='decorations.php';</script>";
    } else {
        echo "<script>alert('Failed to add decoration.');</script>";
    }
}
?>
...
<div class="modern-form-container">
    <h2 class="modern-form-title"> Add New Decoration </h2>
    <form method="POST" enctype="multipart/form-data">
        <div style="display:flex; gap:20px;">
            <div class="modern-form-group" style="flex:2;">
                <label>Decoration Theme *</label>
                <input type="text" name="name" class="modern-input" required placeholder="e.g. Birthday Party Balloons">
            </div>
            <div class="modern-form-group" style="flex:1;">
                <label>Selling Price (₹) *</label>
                <input type="number" name="price" class="modern-input" required placeholder="e.g. 4999">
            </div>
            <div class="modern-form-group" style="flex:1;">
                <label>Original Price (₹)</label>
                <input type="number" name="original_price" class="modern-input" placeholder="e.g. 5999">
            </div>
        </div>
        ...
        <div class="modern-form-group">
            ...
            <textarea name="description" class="modern-input modern-textarea"
                placeholder="What's included in this decoration?"></textarea>
        </div>

        <button type="submit" name="add_decoration" class="modern-btn-submit">Add Decoration</button>
    </form>
</div>
