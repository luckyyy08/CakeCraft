...
if(!isset($_GET['id'])) {
    echo "<script>window.location.href='hampers.php';</script>";
    exit;
}

$id = intval($_GET['id']);
$item_query = mysqli_query($conn, "SELECT * FROM hampers WHERE id = $id");
$item = mysqli_fetch_assoc($item_query);

if(!$item) {
    echo "<script>alert('Hamper not found!'); window.location.href='hampers.php';</script>";
    exit;
}
...
if(isset($_POST['update_hamper'])) {
    ...
    if(mysqli_query($conn, $q)) {
        echo "<script>alert('Hamper updated successfully!'); window.location.href='hampers.php';</script>";
    } else {
        echo "<script>alert('Failed to update hamper.');</script>";
    }
}
?>
...
<div class="modern-form-container">
    <h2 class="modern-form-title">✏️ Edit Gift Hamper ✏️</h2>
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="modern-form-group">
            ...
            <img src="../assets/images/hampers/<?php echo $item['image']; ?>" width="80" style="margin-top:10px; border-radius:5px;">
        </div>
        ...
        <button type="submit" name="update_hamper" class="modern-btn-submit">Update Hamper</button>
    </form>
</div>
