...
if (isset($_POST['add_candle'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        move_uploaded_file($tmp_name, "../assets/images/candles/" . $image);
    }

    $q = "INSERT INTO candles (name, price, original_price, image, description) VALUES ('$name', '$price', '$original_price', '$image', '$description')";

    if (mysqli_query($conn, $q)) {
        echo "<script>alert('Candle added successfully!'); window.location.href='candles.php';</script>";
    } else {
        echo "<script>alert('Failed to add candle.');</script>";
    }
}
?>
...
<div class="modern-form-container">
    <h2 class="modern-form-title"> Add New Candle </h2>
    <form method="POST" enctype="multipart/form-data">
        <div style="display:flex; gap:20px;">
            <div class="modern-form-group" style="flex:2;">
                <label>Candle Name *</label>
                <input type="text" name="name" class="modern-input" required placeholder="e.g. Scented Lavender Candle">
            </div>
            <div class="modern-form-group" style="flex:1;">
                <label>Selling Price (₹) *</label>
                <input type="number" name="price" class="modern-input" required placeholder="e.g. 199">
            </div>
            <div class="modern-form-group" style="flex:1;">
                <label>Original Price (₹)</label>
                <input type="number" name="original_price" class="modern-input" placeholder="e.g. 299">
            </div>
        </div>
        ...
        <div class="modern-form-group">
            ...
            <textarea name="description" class="modern-input modern-textarea"
                placeholder="Details about scent, color, etc."></textarea>
        </div>

        <button type="submit" name="add_candle" class="modern-btn-submit">Add Candle</button>
    </form>
</div>
